...
class OpenArm:
    def __init__(self, device_path):
        ...
        self.read_only = True
        self.DOF = 8
        assert self.DOF == len(self.port.motors), f"DOF {self.DOF} != {len(self.port.motors)}"
    ...
    def safe_set_position(self, radians):
        """Use hardcoded K values for slow movement"""
        for i, rad in enumerate(radians):
            self.port.controlMIT(i, 1.0, 0.1, rad, 0.0, 0.0)

    def set_position(self, radians):
        for i, rad in enumerate(radians):
            self.port.controlMIT(i, self.KP[i], self.KD[i], rad, 0.0, 0.0)

        return True

    def get_state(self):
        if self.read_only:
            # refresh the motor state
